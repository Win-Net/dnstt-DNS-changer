#!/bin/bash
# ═══════════════════════════════════════════════════════════
# DNSTT-DNS-Changer v2.1.0 - Fast Scanner
...
auto_scan() {
    ...
    log "INFO" "AUTO-SCAN: Starting fast scan for $scan_count DNS servers"
    ...
    log "INFO" "AUTO-SCAN: Testing ${#SCAN_LIST[@]} DNS with dnstt (~5-10s each)"
    ...
    for ip in "${SCAN_LIST[@]}"; do
        ...
        if [ $((tested % 10)) -eq 0 ]; then
            log "INFO" "AUTO-SCAN: Progress: $tested tested, $found found, $skipped skipped"
        fi
    ...
    if [ $found -lt $scan_count ]; then
        log "INFO" "AUTO-SCAN: Random scanning (need $((scan_count - found)) more)"
        local extra=0
        while [ $found -lt $scan_count ] && [ $extra -lt 100 ]; do
